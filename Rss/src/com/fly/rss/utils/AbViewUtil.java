...
/**
 * 对应继承自AdapterView的Layout来说,每一个Item�?��显示的高度是由minHight决定的，默认只是布局高度�? * 如果要调整item的显示高度则�?��调用scale()方法，得到缩放�?，然后在getView()中手动设�?
 * 没一个item内部的布�?��要调用scaleContentView()设置�?��. 名称：AbViewUtil.java 描述：View工具�?
 */
public class AbViewUtil {
	...
	/**
	 * 描述：重置AbsListView的高�? item 的最外层布局要用
	 * RelativeLayout,如果计算的不准，就为RelativeLayout指定�?��高度
	 * 
	 * @param absListView
	 *            the abs list view
	 * @param lineNumber
	...
	/**
	 * 测量这个view，最后�?过getMeasuredWidth()获取宽度和高�?
	 * 
	 * @param v
	 *            要测量的view
	 * @return 测量过的view
	...
	public static int scale(int displayWidth, int displayHeight, float pxValue) {
		...
		try {
			...
			float scaleHeight = (float) displayHeight / UI_HIGHT;
	...
	/**
	 * 
	 * 描述：View树�?归调用做适配. AbAppConfig.uiWidth = 1080; AbAppConfig.uiHeight = 700;
	 * scaleContentView((RelativeLayout)findViewById(R.id.rootLayout));
	 * 要求布局中的单位都用px并且和美工的设计图尺寸一致，包括�?��宽高，Padding,Margin,文字大小
	 * 
	 * @param contentView
	 */
	 ...
	 * 
	 * @param view
	 *            如果是代码new出来的View，需要设置一个�?合的LayoutParams
	 * @param widthPixels
	 * @param heightPixels
	 */
	public static void setViewSize(View view, int widthPixels, int heightPixels) {
		...
		if (params == null) {
			Log.e(AbViewUtil.class.getSimpleName(),
					"setViewSize出错,如果是代码new出来的View，需要设置一个�?合的LayoutParams");
			return;
		}
	 ...
	 *            对象
	 * @param dividerHeight
	 *            间隔线高�?	 * @param numColumns
	 *            分了多少�?	 */
	public static void setAbsListViewHeightBasedOnChildren(
			AbsListView absListView, int dividerHeight, int numColumns) {
		...
		if (listAdapter.getCount() % numColumns == 0) {
			itemCount = listAdapter.getCount() / numColumns;
		} else {
			itemCount = listAdapter.getCount() / numColumns + 1;
		}
		...
		ViewGroup.LayoutParams params = absListView.getLayoutParams();
		params.height = totalHeight + (dividerHeight * (itemCount - 1));
	 ...
	 *            间隔线高�?	 * @param numColumns
	 *            分了多少�?	 */
	public static void setAbsListViewHeightBasedOnCount(
			AbsListView absListView, int dividerHeight, int count) {
		ListAdapter listAdapter = absListView.getAdapter();
		if (listAdapter == null) {
			// pre-condition
			return;
		...
		ViewGroup.LayoutParams params = absListView.getLayoutParams();
		params.height = totalHeight + (dividerHeight * (count - 1));
		//System.out.println("�?" + params.height);
		absListView.setLayoutParams(params);
	}
