...
public class RSSParser implements RSSParserSPI {
  ...
  public RSSParser(RSSConfig config,ResponseDeliver deliver) {
    this.config = config;
  ...
  private RSSFeed parse(SAXParser parser, InputStream feed)
    ...
    if (parser == null) {
    	if(mDeliver != null) mDeliver.deliverError(new RSSException("RSS feed must not be null."));
    } else if (feed == null) {
    	if(mDeliver != null) mDeliver.deliverError(new RSSException("RSS feed must not be null."));
    }
