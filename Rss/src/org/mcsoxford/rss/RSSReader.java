...
public class RSSReader implements java.io.Closeable{
  ...
  private ResponseDeliver mDeliver = null;	
   ...
   */
  public RSSReader(HttpClient httpclient, RSSParserSPI parser,ResponseDeliver deliver) {
   ...
   */
  public RSSReader(HttpClient httpclient, RSSConfig config,ResponseDeliver deliver) {
    this(httpclient, new RSSParser(config,deliver),deliver);
  }
   ...
   * {@link RSSConfig}.
   */
  public RSSReader(RSSConfig config,ResponseDeliver deliver) {
    this(new DefaultHttpClient(), new RSSParser(config,deliver),deliver);
  }
   ...
   */
  public RSSReader(ResponseDeliver deliver) {
    this(new DefaultHttpClient(), new RSSParser(new RSSConfig(),deliver),deliver);
  }
 ...
 * @throws ClientProtocolException 
  ...
  public RSSFeed load(String uri){
	  ...
	  try {
	      ...
	      if (status.getStatusCode() != HttpStatus.SC_OK) {
	    	  if(mDeliver != null){
	    		  mDeliver.deliverError(new RSSReaderException(status.getStatusCode(),status.getReasonPhrase()));
	    	  }
	      }
	      ...
	      if (feed.getLink() == null) {
	    	...
	    	if(mDeliver != null){
	    		  mDeliver.deliverError(new RSSException(e));
	    	}
	    } catch (IOException e) {
	    	if(mDeliver != null){
	    		  mDeliver.deliverError(new RSSException(e));
  ...
  public void setOnResponseDeliver(ResponseDeliver deliver){
	  mDeliver = deliver;
  }
